...
const tokendb = new Database("../Dashboard/Json-Database/Others/Tokens.json");
const rundb = new Database("../Dashboard/Json-Database/BotMaker/RunBots.json");
...
tokens.forEach(async (token) => {
	...
	const commandsdb = new Database(`../Dashboard/Json-Database/CommandsData/BotsCommands_${token.ClientID}.json`);
	...
	setInterval(async () => {
		if (client && client.user) {
			const localData = new Database(`../Dashboard/Json-Database/BotsLocalData/LocalData_${client.user.id}.json`);
			let botGuilds = []
	...
	const Botlanguage = new Database("../Dashboard/Json-Database/Others/Language.json");
	const logdb = new Database("/Json-Database/Systems/Log.json");
	const protectiondb = new Database("/Json-Database/Settings/Protection.json")
		...
		if (Array.isArray(botIDArray) && botIDArray.length > 0) {
			...
			if (botIDArray) {
				if (tokens) {
					...
					const commandsdb = new Database(`../Dashboard/Json-Database/CommandsData/BotsCommands_${botID}.json`);
					const { Client, Collection, Discord, ButtonStyle, ButtonBuilder, ActionRowBuilder, createInvite, EmbedBuilder, ChannelType, ActivityType, WebhookClient, PermissionsBitField, GatewayIntentBits, Partials, ApplicationCommandType, ApplicationCommandOptionType, Events, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ContextMenuCommandBuilder, SlashCommandBuilder, REST, Routes, GatewayCloseCodes } = require('discord.js');
					const { readdirSync } = require("fs");
					const client = new Client({
						intents: [
